...
var locale_select: EditorLocaleDialog = null
...
func _translation_res_add(p_paths: PackedStringArray) -> void:
	...
	for path in p_paths:
		if !remaps.has(path):
			...
			remaps[path] = PackedStringArray.new()
...
func _translation_res_option_popup(p_arrow_clicked: bool) -> void:
...
func connect_filesystem_dock_signals(p_fs_dock: FileSystemDock) -> void:
	p_fs_dock.connect("files_moved", self, "_filesystem_files_moved")
	p_fs_dock.connect("file_removed", self, "_filesystem_file_removed")
	...
	if ProjectSettings.has_setting("internationalization/locale/translation_remaps"):
		...
		for i in range(keys.size()):
			...
			if !FileAccess.exists(keys[i]):
				...
				t.set_tooltip_text(0, keys[i] + TTR(" cannot be found."))

			if keys[i] == remap_selected:
				t.select(0)
				translation_res_option_add_button.disabled = false

				var selected: PackedStringArray = remaps[keys[i]]
				for j in range(selected.size()):
					...
					if !FileAccess.exists(path):
						...
						t2.set_tooltip_text(0, t2->get_tooltip_text(0) + TTR(" cannot be found."))
...
func _init() -> void:
	...
	translations.v_size_flags = Control.SIZE_EXPAND_FILL
	...
	addtr.text = TTR("Add...")
	addtr.connect("pressed", self, "_translation_res_file_open")
	thb.add_child(addtr)

	var tmc: VBoxContainer = VBoxContainer.new()
	tmc.v_size_flags = Control.SIZE_EXPAND_FILL
	tvb.add_child(tmc)

	translation_remap = Tree.new()
	translation_remap.v_size_flags = Control.SIZE_EXPAND_FILL
	translation_remap.connect("cell_selected", self, "_translation_res_select")
	translation_remap.connect("button_clicked", self, "_translation_res_delete")
	...
	translation_res_file_open_dialog.connect("files_selected", self, "_translation_res_add")
	...
	addtr = Button.new()
	addtr.text = TTR("Add...")
	addtr.connect("pressed", self, "_translation_res_option_file_open")
	...
	translation_remap_options.v_size_flags = Control.SIZE_EXPAND_FILL
	...
	translation_remap_options.connect("item_edited", self, "_translation_res_option_changed")
	translation_remap_options.connect("button_clicked", self, "_translation_res_option_delete")
	translation_remap_options.connect("custom_popup_edited", self, "_translation_res_option_popup")
	...
	translation_res_option_file_open_dialog.connect("files_selected", self, "_translation_res_option_add")
